...
class Compat {
    ...
    public static function typeof(v:Dynamic) : String {
     ...
     public static inline function hasOwnProperty(obj: Dynamic, field: String): Bool {
        #if flash 
           return obj.hasOwnProperty(field);
        #else  
        var fields: Array<String> = [];
        var objType: ValueType = Type.typeof(obj);
        switch (objType) {
            case TClass(c):
    ...
    public static inline function vectorSplice<T>(a:openfl.Vector<T>, startIndex:Int, deleteCount:Int, ?values:openfl.Vector<T>):openfl.Vector<T> {
        var result = a.splice(startIndex, deleteCount);
        if(values != null) {
            for(i in 0...values.length) {
...
class FlashRegExpAdapter {
    ...
    public var global(get,null) : Bool;
